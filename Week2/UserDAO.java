...
public class UserDAO {
    ...
    public DBObject validateLogin(String username, String password) {
     ...
     //   DBCursor cursor = usersCollection.find(new BasicDBObject("_id",username),new BasicDBObject("_id",true));
        
        DBCursor cursor = usersCollection.find(new BasicDBObject("_id",username));
		try{
			while(cursor.hasNext()){
				user = cursor.next();
				System.out.println(user+"Ahoy!");
			}
		}
		finally{
			cursor.close();
